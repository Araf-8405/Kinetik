...
export default class testData {
        ...
        async getFirstname() {
                return this.firstNames[this.randomNumber];
        }
        ...
        //     return cy.readFile('./counter.txt').then(data => {
        //             return parseInt(data);
        //         }
        //     );
        // }

        // async writeCounter(counter) {
        //     // const fileSystem = require('fs');
        //     // const filePath = './counter.txt';
        //     // fileSystem.writeFileSync(filePath, String(counter + 1));
        //     cy.writeFile('counter.txt', String(counter + 1));
        // }

        // async writeCounterFile() {
        ...
        async generateEmailAlias(emailPrefix: string, counter: number, emailSuffix: string) {
                return emailPrefix + "+" + counter + emailSuffix;
        }
        async generateRandomName(): Promise<string> {
                const prefixes = ['john', 'emma', 'alex', 'sophie', 'david', 'olivia', 'michael', 'emily'];
                const suffixes = ['smith', 'johnson', 'miller', 'brown', 'wilson', 'taylor', 'martin', 'davis'];
              
                const randomPrefix = prefixes[Math.floor(Math.random() * prefixes.length)];
                const randomSuffix = suffixes[Math.floor(Math.random() * suffixes.length)];
              
                return `${randomPrefix}.${randomSuffix}@kinetik.com`;
        ...
        getInvalidPasswordForLogin() {
                return "invalidName"
        }
        getColorCode() {
                return "rgb(117, 161, 64)"
        }
        getTextColorCode() {
                return "rgb(29, 168, 101)"
        }

        inputTextColorCode() {
                return "1DA865FF"
        }
        inputLoadingTextColorCode() {
                return "1DA865FF"
        }
        postName() {
                return "Custom Adds"
        ...
        inputPopUpBackgroundColorCode() {
                return "1DA865FF"
        }
        inputPopUpTextColorCode() {
                return "FFFFFFFF"
        }
        getPopUpBackgroundColorCode() {
                return "rgb(29, 168, 101)"
        }
        getPopUpTextColorCode() {
                return "rgb(255, 255, 255)"
        }
        inputTextSubmitHighlightColorCode() {
                return "1DA865FF"
        }
        getButtonTextColorCode() {
                return "rgb(33, 41, 54)"
        }
        colorCodeForPostUP() {
                return "5FBCD2FF"
        ...
        getAppliedColorCodeForButton() {
                return "rgb(253, 124, 106)"
        }

        getGameTitle() {
                return "Demo Game Title"
        }
        maxMiniText() {
                return "0/35 Characters"
        }
        ...
        getQuestionText() {
                return "Demo Question Header"
        }

        getCountDownTitle() {
                return "Demo CountDown Title"
        }
        getLeaderBoardTitle() {
                return "Demo Leaderboard Title"
        }
        countDownMaxMiniText() {
                return "0/120 Characters"
        }
        leaderBoardMaxMiniText() {
                return "0/120 Characters"
        }
        getRoundName() {
        ...
        getNameFieldErrorMassage() {
                return "Field cannot be empty. Please type something"
        }
        getNameFieldDublicateErrorMassage() {
                return "Instance with the same name already exists. Please enter a unique name."
        }
        getPreeEventMassage() {
                return "Pree Event Massage"
        }
        getWelcomeMassage() {
                return "Welcome Massage"
        }
        getPostSubmitMassage() {
                return "Post Submit"
        }
        getLoadingMassage() {
                return "Loading Massage"
        }
        getPostTextPlaceholderMassage() {
                return "Post Text Placeholder"
        }

        public getRandomText =(wordLength: number): string => {
        ...
        getVideoMinMaxText() {
                return "The video must be (5)-(15) seconds long"
        }
